...
//tokenize each sweep
    ...
    //--- end sweep found or NOT
        ...
        echo "<img src=\"images/btnInsert.jpg\" width=\"46\" height=\"15\" border=\"0\"></button>\n";
      if($sweepID != "0") {
            echo "<button style=\"border:0; background-color: transparent\" onClick=\"clearBtn($ID,$areaID,$sweepID)\" >";
//            echo "<button style=\"border:0; background-color: transparent\" name=\"clear\" value=\"$sweepID\" >";
            echo "<img src=\"images/btnClear.jpg\" width=\"46\" height=\"15\" border=\"0\"></button>\n";
      }
        echo "</tr>\n";
       $tok = strtok(" ");
...
var assID = document.myForm.assignments.selectedIndex;
    ...
    if(assID < 0)
        ...
        var url="hill_assignments.php?insert="+sweepID+"&patrollerID="+patrollerID+"&areaID="+areaID;
        alert(url);
...
?>
<div style="height:auto;width:785px">
  <table border="1" width="785" cellspacing="0" cellpadding="0">
    <tr>
    ...
    <tr>
      <td width="180" bgcolor="#EBEBEB" align="center"><font size="2"><b>Early Sweep</b></font></td>
...
//$foo = DateTimeZone::listIdentifiers();
//for($i=1; $i < 50; $i++)
// echo $foo[$i] . "<br>";
    $arrDate = getdate();
...
echo "(aredId=$areaID, patrollerID=$patrollerID, AutoFill=$AutoFill, clear=$clear, insert=$insert)";
...
//tokenize each sweep
    ...
    while ($tok) {
       ...
       $query_string = "SELECT * FROM sweepdefinitions WHERE id=$sweepID";
echo "$query_string<br>";
       $result = @mysql_db_query($mysql_db, $query_string) or die ("Invalid query 12");
        if ($result && $row = @mysql_fetch_array($result)) {
...
//????? why is this here????  exit;
}

//build list of sweepsAssigned[idOfSweep] = IdOfPatroller
...
//echo "$query_string<br>";
    ...
    while ($row = @mysql_fetch_array($result)) {
        $listOfSweepIds = $row[sweep_ids];
        //loop through all sweep tokens in the list
        $sweepId = strtok($listOfSweepIds, " ");
        while ($sweepId) {
            $sweepsAssigned[$sweepId] = $row[patroller_id];
           $sweepId = strtok(" ");
        }
    }
    ...
    $xtraName=array();
    $xtraSweeps=array();
    //just look at DAY shifts (shift 0)
...
?>
<div style="overflow:auto;height:300px;width:797px">
  <table border="1" width="785" cellspacing="0" cellpadding="0">
<?
...
<table border="1" width="450"  bgcolor="#EBEBEB" >
  ...
  <tr>
    <td>
      <select size="5" name="assignments" style="width:540px">
<?
    $sel="SELECTED";
