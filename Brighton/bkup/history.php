...
//echo "name=$name";
	...
	if($millis) {
		...
		$strToday .= " (about $daysOld day(s) old)";
	}
}
else if (!admin)
...
//alert ("show history for patroller ID=" + id);
    ...
    if(id > 0 && (evt == null || (evt.keyCode == 13 || evt.keyCode == 32))) {
        window.location="history.php?admin=1&ID="+id;
    }
}
...
if($audit) {
    ...
    echo "<b>$name</b>, as of $strToday</p></font>\n";
...
while ($row = @mysql_fetch_array($result)) {
    ...
    $value  /= 2;  //day, swing and night before 4 pm = 4, night before 5:30=3, night=2
	$simpleCredits += $value;
...
if(!$audit)
  ...
  echo "  <td align=\"center\">" . number_format($multiplier, 2, '.', ',') . "</td>\n";           //Multiplier
  echo "  <td align=\"center\">" . number_format($credit, 2, '.', ',') . "</td>\n";
  if($admin) echo "<td align=\"center\"><a href=\"edit_history.php?ID=$ID&edit=$history_id\"><img border=\"0\" src=\"Edit.jpg\" width=\"27\" height=\"14\"></a></td>";
  echo "</tr>\n";
}

  echo "<tr>\n";
  echo "  <td align=\"center\" colspan=\"8\">&nbsp;</td>\n";
  if($admin && $ID) echo "<td align=\"center\"><a href=\"edit_history.php?ID=$ID&add=1\">Add</a></td>";
  else if($admin) echo "<td>&nbsp</td>";
    ...
    echo "<td align=center colspan=2 bgcolor=\"#FFFF00\">$totalNights Nights</td>\n";
