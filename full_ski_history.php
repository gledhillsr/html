...
// DisplayLoginHistory
//=======================
function DisplayLoginHistory($name,$commitment) {
global $showDetails,$showCommitment,$startingTicks,$endingTicks,$mysql_db, $dataFrom;
...
//echo "$query_string<br>";
    $result1 = @mysql_db_query($mysql_db, $query_string) or die ("Invalid query");
	$count = 0;
	$loginDetail = "";
...
//			if($showDetails) {
				...
				if($showDouble) {
					$dow = date("n/j", $row1[date]);	//display date as only day of week, ie" "Saturday"
...
global $showDetails,$showCommitment,$startingTicks,$endingTicks,$mysql_db;
global $showDay, $showSwing, $showNight, $strBeginning, $strEnding, $showWeekday, $showDouble;
//print input field area
?>
<form method="POST" name=myForm action="full_ski_history.php">
...
<table  style="{background-color:#FFFFFF; border-width:1px; border-color:#FFFFFF; border-collapse:collapse; border-spacing:0" border="0" cellpadding="2"  width=580 >
  <tr>
    ...
    <td  style="{background-color:#FFFFFF; border-width:1px; border-color:#FFFFFF; border-collapse:collapse; border-spacing:0">
        ...
        Display:&nbsp; <input type="checkbox" name="showDetails" value="ON" <? if($showDetails) echo "checked"; ?> >Detail of Shifts
        			   ...
        			   <input type="checkbox" name="showDouble" value="OFF" <? if($showDouble) echo "checked"; ?> >Show Double Days in bold&nbsp; 
        				<br>
        Display:&nbsp; <input type="checkbox" name="showCommitment" value="ON" <? if($showCommitment) echo "checked"; ?> >Commitment (Full Time / Part Time)<br>
        <br>
        Sort By:&nbsp; <input type="radio" value="first"  name="sortBy" <? if($sortBy == "first") echo "checked"; ?> >First Name&nbsp;&nbsp;&nbsp; 
        			   <input type="radio" value="last"   name="sortBy" <? if($sortBy == "last")  echo "checked"; ?> >Last Name&nbsp;&nbsp;&nbsp; 
        			   <input type="radio" value="shifts" name="sortBy" <? if($sortBy == "shifts")echo "checked"; ?> >Shifts Skied (
  ...
  Use Data From:&nbsp; <input type="radio" value="locker" name="dataFrom" <? if($dataFrom == "locker") echo "checked"; ?> >Locker&nbsp;&nbsp;&nbsp; 
        			   ...
        			   <input type="radio" value="diff"   name="dataFrom" <? if($dataFrom == "diff")   echo "checked"; ?> >Diff of Both&nbsp;&nbsp;&nbsp; 
...
global $query_string2,$mysql_db, $loginDetail, $dataFrom, $showCommitment, $showDetails;
//dataFrom
//"locker" 
//"web"    
//"sum"    
...
if($dataFrom == "locker") return;
	buildQueryString2($ID);
  	$result = @mysql_db_query($mysql_db, $query_string2) or die ("Invalid query");
	$cnt = 0;
	$date1 = "";
  	while ($row = @mysql_fetch_array($result)) {
		$date2 = $row[Date];
		$date2 = substr($date2,5,2) . "/" . substr($date2,8,2) . "/" . substr($date2,0,4) ;
		if($dataFrom == "diff") {
...
global $chg,$srtLastPrinted,$strToday, $sortBy, $query_string1, $query_string1, $showDouble;
global $showDetails,$showCommitment,$startingTicks,$endingTicks,$mysql_db, $dataFrom;
...
//display table contents
	...
	if($sortBy == "first" || $sortBy == "last") {
		if($sortBy == "first")
	    	$query_string = "SELECT * FROM roster WHERE 1 ORDER BY FirstName";
		else if($sortBy == "last")
	    ...
	    while ($row = @mysql_fetch_array($result)) {
			...
			DisplayLoginHistory($name,$commitment);
			DisplayWebHistory($name,$commitment,$ID);
	    }
	} else { //($sortBy == "shifts" ) 
	    $result1 = @mysql_db_query($mysql_db, $query_string1) or die ("Invalid query 2");
        $patrollers=array();
	    while ($row1 = @mysql_fetch_array($result1)) {
	        $name 	   = $row1[name];
			$pos = strpos($name," ");
