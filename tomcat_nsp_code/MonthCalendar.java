...
synchronized (this) {
        ...
        realCurrYear = 0;
		realCurrDate = 0;
        Names = new Hashtable(MemberData.MAX_MEMBERS);

        String str = request.getParameter("noLogin");
		str = request.getParameter("textLen");
        if(str != null && !str.equals("") ) {
            Integer intgr = new Integer(str);
            textLen = intgr.intValue();
		} else
			textLen = 0;	//undefined

        str = request.getParameter("noLogin");
        strID = request.getParameter("ID");
        noLogin = false;
        if(str != null && !str.equals("") ) {
            //nlLogin does exist
            if(strID == null || strID.equals("") )  //ID does NOT exist
               noLogin = true;
        }
...
//System.out.println("szYear=("+szYear+")");
        if(szMonth != null && szYear != null) {
            Integer intgr = new Integer(szMonth);
            currMonth = intgr.intValue();
            intgr = new Integer(szYear);
            currYear = intgr.intValue();
//    currMonth =11; //0 based month
        }
        str = request.getParameter("FontSize");
//System.out.println("FontSize=("+str+")");
        if(str != null && !str.equals("")) {
            textFontSize = cvtToInt(str);
...
//      monthData = new int[32][Assignments.MAX]; //all shifts for all days in 1 month
        monthData = new Assignments[32][Shifts.MAX]; //all shifts for all days in 1 month
...
// readData
//---------------------------------
    public void readData(String resort) {
        int year,month,day, pos;
        MemberData member;
        Assignments assignments;
        java.util.Date date1;

        PatrolData patrol = new PatrolData(PatrolData.FETCH_ALL_DATA,resort); //when reading members, read full data
        ds = patrol.readDirectorSettings();
//build hash table of all members
        while((member = patrol.nextMember("")) != null) {
            Names.put(new Integer(member.idNum), member.getFullName()+", "+member.getHomePhone());
...
//build array of assignments for a SINGLE month using currYear & currMonth
        ...
        while((shift = patrol.readNextShift()) != null) {
            ...
            if(name.equals("Sunday"))         newDay = Calendar.SUNDAY;
            else if(name.equals("Monday"))    newDay = Calendar.MONDAY;
            else if(name.equals("Tuesday"))   newDay = Calendar.TUESDAY;
            else if(name.equals("Wednesday")) newDay = Calendar.WEDNESDAY;
            else if(name.equals("Thursday"))  newDay = Calendar.THURSDAY;
            else if(name.equals("Friday"))    newDay = Calendar.FRIDAY;
            else if(name.equals("Saturday"))  newDay = Calendar.SATURDAY;
            else                              newDay = -1;
...
//cancel button pressed
        ...
        String szLogin = "MonthCalendar?resort="+resort+strID+"&month="+currMonth+"&year="+currYear;
...
//insert page for previous button
    String szPrevHTML = "MonthCalendar?resort="+resort+strID+"&month="+prevMonth+"&year="+prevYear;
    if(noLogin)
...
//insert page for next button
    String szNextHTML = "MonthCalendar?resort="+resort+strID+"&month="+nextMonth+"&year="+nextYear;
    if(noLogin)
...
//  if(currMonth != realCurrMonth) {
        String szCurrHTML = "MonthCalendar?resort="+resort+strID+"&month="+realCurrMonth+"&year="+realCurrYear;
        if(noLogin)
...
String go = "MonthCalendar?resort="+resort+strID;    //hack
if(szMonth != null)
...
// onSelect=window.location=\"welcome.html\"
	...
	if( denseFormat ) {
		go = "MonthCalendar?resort="+resort+strID;  
		if(szMonth != null)
		    go += "&month="+szMonth+"&year="+szYear;
//		go += "&textLen=";
...
//build hyperlink tags
        String htmData = "ChangeShift?resort="+resort+strID+"&dayOfWeek="+(dayOfWeek-1)+"&date="+day+"&month="+currMonth+"&year="+currYear+"&pos=";
        String htmDisplayData = "<a href=\"DayShifts?resort="+resort+"&dayOfWeek="+(dayOfWeek-1)+"&date="+day+"&month="+currMonth+"&year="+currYear+ strID + "\" Title=\"Shift Details..\"><B><font face=\"arial,helvetica\" color=\"#0000FF\" size=4>"+new Integer(day).toString()+"</font></B></a>";
        if(noLogin)
            htmDisplayData = "<B><font face=\"arial,helvetica\" color=\"#0000FF\" size=4>"+new Integer(day).toString()+"</font></B>";
                ...
                if(!inSeason) {
                    ...
                    for(i=0; i < Shifts.MAX; ++i) {
                        ...
                        for(int j=0; j < shift.getCount() && !noLogin; ++j) {
                            printName(out,htmData+((i+1)+"&index="+j), "TOP",0,shift.getStartString(),day,dayOfWeek,count++,resort);
                        }
                    }
                    out.println("</table>");
                }
...
//                out.println(eventName);
            ...
            out.println("<TABLE WIDTH=\"100%\" Border=\"0\" CELLPADDING=\"0\" CellSpacing=\"0\">");
            int posCount = 0;
            htmData = "ChangeShift?resort="+resort+"&dayOfWeek="+(dayOfWeek-1)+"&date="+day+"&month="+currMonth+"&year="+currYear+ strID + "&pos=";
...
//                    String time = "("+data[day][i].getStartingTimeString()+") ";
                    ...
                    try {
                        ...
                        printName(out,htmData+((i+1)+"&index="+j), "TOP",id,time,day,dayOfWeek,posCount++,resort);
                    } catch (Exception e) {
...
// printName
//------------
    public void printName(PrintWriter out,String html, String tag,int id, String time, int day, int dayOfWeek,int posIndex,String resort) {
        String QuickTip;
        String szName;
...
//          if(denseFormat)
//              time = "";
            QuickTip = (String)Names.get(new Integer(id)); //format ,"Steve Gledhill,(801) 571-7716"
            if(QuickTip == null)
                szName = "Err id ("+id+")";
            else if (missed) {
/*full name*/     szName = time+"<strike>" + QuickTip.substring(0, QuickTip.lastIndexOf(',')) + "</strike>";
// first name     szName = time+"<strike>" + QuickTip.substring(0, QuickTip.indexOf(' ')) + "</strike>";
            } else {
/*full name*/     szName = time+QuickTip.substring(0, QuickTip.lastIndexOf(','));
...
//          szRowStart = "<TR><TD NOWRAP>";
        ...
        if(isDirector || 
		 (ds.getDirectorsOnlyChange() == false && validDateToEdit)) {
//            out.println(szRowStart+"<a href=\""+html+"\" Title=\""+QuickTip+"\"><font face=\"arial,helvetica\" color=\"#0000FF\">"+szName+"</font></a>"+szRowEnd);
            out.println(szRowStart+"<a href=\""+html+"\" Title=\""+QuickTip+"\">"+szName+"</a>"+szRowEnd);
        } else {
