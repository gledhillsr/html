...
synchronized (this) {
        ...
        ID = cookie.getID();    //editor's ID
        if(ID != null) {
            readData(ID); //to get director (ID from cookie)
...
//      PreparedStatement sRost = c.prepareStatement("SELECT * FROM Roster ORDER BY \"FirstName\",\"LastName\"");
//      ResultSet rs = sRost.executeQuery();
      rosterSize = 0;
      MemberData member;
      while ( (member = patrol.nextMember("")) != null ) {
//        sortedRoster[rosterSize++] = member.getFullName();
        sortedRoster[rosterSize++] = member.getLast() + ", " + member.getFirst();
//System.out.println("'sortedRoster[rosterSize]'="+idNum);
//        NameToNum.put(new String(sortedRoster[rosterSize]),new Integer(idNum));
//        NumToName.put(new Integer(idNum),sortedRoster[rosterSize]);
//        rosterSize++;
      }
        if(ID.equalsIgnoreCase(PatrolData.bdLogin)) {
            isDirector=true;
        } else {
...
//            nextPage = PatrolData.SERVLET_URL+"CustomizedList?resort="+resort+"&ID="+ID;
            nextPage = "CustomizedList?resort="+resort+"&ID="+ID;
...
//            nextPage=PatrolData.SERVLET_URL+"UpdateInfo?resort="+resort+"&Purpose=Add_Patroller&ID="+ID;
            nextPage="UpdateInfo?resort="+resort+"&Purpose=Add_Patroller&ID="+ID;
...
//            nextPage = PatrolData.SERVLET_URL+"CustomizedList2?resort="+resort+"&CLASS=1&AUX=1&BAS=1&SR=1&SRA=1&TRA=1&CAN=1&PRO=1&ALM=1&INA=1&FullTime=1&PartTime=1&Inactive=1&FIRST=1&DAY_CNT=1&DAY_DETAILS=1&ALL=1&FirstSort=shiftCnt&SecondSort=None&ThirdSort=None&FontSize=12&ID="+ID;
            nextPage = "CustomizedList2?resort="+resort+"&NAME=LAST&AUX=1&BAS=1&SR=1&SRA=1&TRA=1&CAN=1&PRO=1&ALM=1&INA=1&FullTime=1&PartTime=1&Inactive=1&FIRST=1&DAY_CNT=1&DAY_DETAILS=1&ALL=1&FirstSort=shiftCnt&SecondSort=None&ThirdSort=None&FontSize=12&ID="+ID;
            out.println("          <td><input type=\"button\" value=\"Sorted By Day Shifts\"  onClick=window.location=\""+nextPage+"\"></td>");
			out.println("        </tr>");
//swing shift			
			out.println("        <tr>");
            nextPage = "CustomizedList2?resort="+resort+"&NAME=LAST&AUX=1&BAS=1&SR=1&SRA=1&TRA=1&CAN=1&PRO=1&ALM=1&INA=1&FullTime=1&PartTime=1&Inactive=1&FIRST=1&SWING_CNT=1&SWING_DETAILS=1&ALL=1&FirstSort=shiftCnt&SecondSort=None&ThirdSort=None&FontSize=12&ID="+ID;
...
//                disableStatus = "";
            nextPage = PatrolData.SERVLET_URL+"CustomizedList2?resort="+resort+"&NAME=LAST&AUX=1&BAS=1&SR=1&SRA=1&TRA=1&CAN=1&PRO=1&ALM=1&INA=1&FullTime=1&PartTime=1&Inactive=1&FIRST=1&NIGHT_CNT=1&NIGHT_DETAILS=1&ALL=1&FirstSort=shiftCnt&SecondSort=None&ThirdSort=None&FontSize=12&ID="+ID;
...
//            nextPage = PatrolData.SERVLET_URL+"Preferences?resort="+resort+"&ID="+ID;
            nextPage = "Preferences?resort="+resort+"&ID="+ID;
...
//            nextPage = PatrolData.SERVLET_URL+"EditShifts?resort="+resort+"&ID="+ID;
            nextPage = "EditShifts?resort="+resort+"&ID="+ID;
...
//            nextPage = PatrolData.SERVLET_URL+"PurgeAssignments?resort="+resort+"&ID="+ID;
            nextPage = "PurgeAssignments?resort="+resort+"&ID="+ID;
