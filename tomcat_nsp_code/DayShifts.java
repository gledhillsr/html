...
//------------
    ...
    {
        MemberData editor = null;
        response.setContentType("text/html");
synchronized (this) {
        out = response.getWriter();
if(debug) System.out.println("------- Entering DayShifts --------");
        ...
        PatrolData patrol;
        patrol = new PatrolData(PatrolData.FETCH_ALL_DATA,resort); //when reading members, read full data
        if (IDOfEditor != null)
            editor = patrol.getMemberByID(IDOfEditor); //ID from cookie

        if(IDOfEditor.equalsIgnoreCase(PatrolData.bdLogin))
            isDirector = true;
        else if(editor != null)
...
if(debug) System.out.println("===assignmentSize = ("+assignmentSize+")");
        //get shift=i,pos=j, and name=name
//zzzz
        for(i=0; i < assignmentSize; ++i) {         //loop for each shift
            name = null;
            j = 0;
            data = (Assignments)assignments.elementAt(i); //0 based
...
if(debug) System.out.println("****param="+param+", name=("+name+")***");
                if(name == null)
...
//--------DELETE ALL SHIFTS -------------------------------------
if(debug) System.out.println("***deleting all shifts.  assignmentSize="+assignmentSize);
        for(int j=assignmentSize-1;j >=0; --j) {    //loop from end of list
            data = (Assignments)assignments.elementAt(j); //0 based
...
if(debug) System.out.println("deleteALL button, assignments.size()="+assignments.size());
if(debug) System.out.println("***************************");
        assignments = new Vector(); //forget all existing assignments
...
if(debug) System.out.println("ERROR, shiftToDelete"+shiftToDelete+") was >= assignmentSize("+assignmentSize+")");
            } else {
                //delete starting record, and modify the rest from assignments
                data = (Assignments)assignments.elementAt(shiftToDelete); //get Assignment to delete
                patrol.deleteAssignment(data);
                assignments.remove(shiftToDelete);
                --assignmentSize;
                for(int j=shiftToDelete; j < assignmentSize; ++j) {
                    data = (Assignments)assignments.elementAt(j);
...
//------------
    private void saveShiftInfo(PatrolData patrol, int assignmentSize, int shiftCount) {
        ...
        for (i = 0; i < shiftCount; ++i) {//loop thru shifts on command line
            Shifts sh = (Shifts)todaysData.elementAt(i); //args read in
            String szAssignmentDate;
            if(i < assignmentSize) { //size in database is size of args read in
                assignment = (Assignments)assignments.elementAt(i);
...
//------------
    private void insureAssignmentExists(PatrolData patrol) {
        int assignmentSize = assignments.size();
if(debug) System.out.println("****in insureAssignmentExists, initial assignmentSize = ("+assignmentSize+")");
        if(assignmentSize > 0)
...
//System.out.println("---selectedShift = ("+selectedShift+")");
        ...
        for(int i = 0; i < shifts.size(); ++i) {
            Shifts sData = (Shifts)shifts.elementAt(i);
...
//------------
    private void showName(String id, String time, boolean missed) {
        String name = (String)NumToName.get(id);
        if(name == null)
            name = "&nbsp;";
        out.println("    <tr>");
...
//print date at top
        ...
        if(isDirector) {
            if(selectedShift.equals(""))
                ...
                for(int i=0; i < shifts.size(); ++i) {
                    Shifts shiftData = (Shifts)shifts.get(i);
...
if(assignments.size() >= Shifts.MAX)
	newOK = " disabled";
if (debug) System.out.println("shifts.size() = "+shifts.size());	
if (debug) System.out.println("assignments.size() = "+assignments.size());	
...
//------------
    private void showEditAssignments(int count) {
        if(assignments.size() == 1) {
            Assignments data = (Assignments)assignments.elementAt(0);
            if(data.getCount() == 0)
