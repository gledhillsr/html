...
synchronized (this) {
        ...
        pData = new PatrolData(PatrolData.FETCH_ALL_DATA,resort);
...
//-------------------------
    private void readParameters(HttpServletRequest request) {
        ...
        szdate1 = request.getParameter("date1");
        pos1 = request.getParameter("pos1");
        index1 = request.getParameter("index1");
        listName = request.getParameter("listName");
//System.out.println("submitterID="+submitterID+" transaction="+transaction+" selectedID="+selectedID+ " date1="+szdate1+" pos1="+pos1+" index1="+index1+" listName="+listName);
System.out.println("submitterID="+submitterID+" trans="+transaction+ " date1="+szdate1+" ("+listName+")");
        member1 = null;
        member2 = null;
        night1 = null;
        night2 = null;
        secondID = null;
        submitter = pData.getMemberByID(submitterID);
        if(submitter == null) {
        ...
        int nPos1 =   Integer.parseInt(pos1);
        nIndex1 = Integer.parseInt(index1);
...
//read assignments
        night1 = pData.readAssignment(szdate1);
        if(night1 == null) {
...
//-------------------------
    private void DisplayParameters() {
        ...
        szPos = night1.getStartingTimeString() + " - " + night1.getEndingTimeString();
        String oldID;
        oldID = night1.getPosID(nIndex1);
        dupError=false;
        ...
        if(transaction.equals("insertMyName")) {
            ...
            //
            // PATROLLER MISSED A SHIFT
            //
            transNumber = MISSED_SHIFT;
            String cleanID;
            boolean missedShift = false;
            cleanID = night1.getPosID(nIndex1);
            if(cleanID.charAt(0) == '-' ) {
                missedShift = true;
            }
            ...
            member1 = pData.getMemberByID(cleanID);
            if(member1 == null) {
            ...
            //
            // REPLACE      THE SOMEONE-ELSE IS IN 'listname'
            //
            member1 = pData.getMemberByName2(listName);
            if(member1 == null) {
                out.println("<h1>Error: member "+listName+" not found!</h1><br>");
                return;
            }
            ...
            //
            // REMOVE   person is stored in 'selectedID'
            //
...
//Insert Steve Gledhill (192443)
        ...
        if(transNumber == REPLACE && !oldID.equals("0")) {
            String oldName;
            member2 = pData.getMemberByID(oldID);
            if(member2 == null)
                oldName = oldID;
            else
...
// printBody
//-------------------------
    public void printBody(HttpServletRequest request,HttpServletResponse response) {
        DisplayParameters();
        DirectorSettings ds = pData.readDirectorSettings();
        boolean notifyPatrollers = ds.getNotifyChanges();
        err = true;
        if(night1 != null)
            err = pData.writeAssignment(night1);
        if(!err && night2 != null)
            err = pData.writeAssignment(night2);
        if(dupError) {
...
//          szTrans[transNumber]+" " + newName;// +" at position: "+ pos1; ???? szDays[dayOfWeek1]
            ...
            strChange3 += "Unless, you are NOT a member of the National Ski Patrol, and recieved this email accidently.\n";
...
//System.out.println("ds.getNotifyChanges()="+ds.getNotifyChanges());
            ...
            } else if(submitterID.equals(PatrolData.bdLogin)) { //using back door login, don't send emails
...
//    System.out.println("----------------");
                    while((mbr = pData.nextMember(""))!= null) {
