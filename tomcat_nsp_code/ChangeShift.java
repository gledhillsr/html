...
//  java.sql.Date[] myShiftAssignments;
//    Shifts[] myShiftAssignments; //part of TradeDate
//  int[] myShiftPosition;
    int myShiftCount;
...
//  String szPos[] = {"7:30 am","7:30 am","3:00 pm","3:00 pm"};
...
//------------------
    private void readParameterData(HttpServletRequest request) {
		...
		if(debug) {
			System.out.println("in ChangeShifts...");
			System.out.println("szDay="+szDay);
			System.out.println("szDate="+szDate);
			System.out.println("szMonth="+szMonth);
			System.out.println("szYear="+szYear);
			System.out.println("szPos="+szPos);
			System.out.println("szIndex="+szIndex);
		}
        ...
        } catch (NumberFormatException ex)
        {
            ...
            year=1;
            pos=1;
			if(debug) System.out.println("ERROR, numeric processing exception, using default values");
        }   //err
...
// showName
//------------------
    private void showName(String id,String time, boolean blink) {
        String name = null;
        name = (String)NumToName.get(id); //format ,"Steve Gledhill"
        if(blink) {
            hisName= name;
...
//System.out.println("showName("+id+", "+loc+") is "+name+ " pos="+pos+" empty="+posWasEmpty);
        if (name == null)
            name = "";
//..        if (blink)
//..            out.println("<TR><TD><font face=\"arial,helvetica\" color=\"#0000FF\" size=\"1\">("+time+") "+name+"</font><font face=\"arial,helvetica\" color=\"#FF0000\" size=\"1\"><BLINK>&lt;&lt;&lt;&lt;&lt;</BLINK></font></TD></TR>");
//..        else
//..            out.println("<TR><TD><font face=\"arial,helvetica\" color=\"#0000FF\" size=\"1\">("+time+") "+name+"</font></TD></TR>");
    } //end showName
...
//System.out.println("strDate is a hack");
//strDate = "2001-01-01";
...
try {
//      out.println("<TABLE WIDTH=\"280\" Border=\"1\" CELLPADDING=\"0\" CellSpacing=\"0\" BACKGROUND=\"/nspImages/ncmnthbk.jpg\">");
        for(i=0; i < maxPos; ++i) {
            int count = assignments[i].getCount();
            for(int j = 0; j < count; ++j) {
                String id = assignments[i].getPosID(j);
                if(id.charAt(0) == '-') { // missedShift
                    id = id.substring(1);
                }
                String time = assignments[i].getStartingTimeString()+" - "+assignments[i].getEndingTimeString();
                showName(id,time,((i+1) == pos && j == index));
            }
        }
} catch (Exception e) {
...
//System.out.println("posWasEmpty="+posWasEmpty);
...
//        if(allowEditing) {
////System.out.println("hack, trade shifts was removed for awhile");
...
//System.out.println(calendarDay+"  "+firstRemoveDay);
		boolean validRemoveRange = (firstRemoveDay < calendarDay) || removeAccess == 0;
//System.out.println("valid to remove:"+validRemoveRange);
...
//------------------
    ...
    PatrolData patrol = new PatrolData(PatrolData.FETCH_ALL_DATA,resort); //when reading members, read full data
        ...
        MyName1 = MyName = (String)NumToName.get(szMyID); //format ,"Steve Gledhill"
	 MyName1 = patrol.getMemberByID(szMyID).getFullName();
...
//System.out.println("****** allowEditing = "+allowEditing);
    patrol.resetAssignments();
    patrol.resetRoster();
    sortedRoster = new String[200];
    rosterSize = 0;
    MemberData member;
    isDirector = false;

    if(szMyID.equalsIgnoreCase(PatrolData.bdLogin))
        isDirector = true;
...
//    myShiftAssignments = new Shifts[300]; //assume never more than 300 per season
    ...
    maxPos = 0;
    String lastPos = " ";
    assignments = new Assignments[50];
    Assignments data;
    myShiftCount = 0;
//    assignmentCount = 0;
    while((data = patrol.readNextAssignment()) != null) {
