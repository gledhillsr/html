...
synchronized (this) {
        CookieID cookie = new CookieID(request,response,"CustomizedList",null);
        ...
        PatrolData patrol = new PatrolData(PatrolData.FETCH_MIN_DATA,resort); //when reading members, read full data
        ...
        if(IDOfPatroller.equals(PatrolData.bdLogin)) {
            isDirector = true;
        }
...
//    out.println("<form action=\""+PatrolData.SERVLET_URL+"CustomizedList2\" method=POST>");
    out.println("<form name=form1 action=\"CustomizedList2\" method=POST>");
...
//    }
    ...
    for(i=1; i <= 31; ++i)  out.println("    <option "+((i == StartDay) ? "selected" : "") + ">"+i+"</option>");
    out.println("  </select>&nbsp;&nbsp;<select size=\"1\" name=\"StartMonth\">");
    for(i=1; i <= 12; ++i)  out.println("    <option value=\""+i+"\" "+((i == StartMonth) ? "selected" : "") + ">"+szMonths[i]+"</option>");
    out.println("  </select>&nbsp;&nbsp;<select size=\"1\" name=\"StartYear\">");
    for(i=year-2; i <= year+2; ++i) out.println("    <option "+((i == StartYear) ? "selected" : "") + ">"+i+"</option>");
    out.println("  </select><br> ");

    out.println("Ending:&nbsp;&nbsp;<select size=\"1\" name=\"EndDay\">");
    for(i=1; i <= 31; ++i)  out.println("    <option "+((i == EndDay) ? "selected" : "") + ">"+i+"</option>");
    out.println("  </select>&nbsp;&nbsp;<select size=\"1\" name=\"EndMonth\">");
    for(i=1; i <= 12; ++i)  out.println("    <option value=\""+i+"\" "+((i == EndMonth) ? "selected" : "") + ">"+szMonths[i]+"</option>");
    out.println("  </select>&nbsp;&nbsp;<select size=\"1\" name=\"EndYear\">");
    for(i=year-2; i <= year+2; ++i) out.println("    <option "+((i == EndYear) ? "selected" : "") + ">"+i+"</option>");
    out.println("  </select><br>");
...
//    }
    ...
    for(i=1; i <= 20; ++i)  out.println("    <option "+((i == 6) ? "selected" : "") + ">"+i+"</option>");
...
//    out.println("          Font Size:   <input type=\"radio\" name=\"FontSize\" value=14 checked>Normal");
//    out.println("          &nbsp;&nbsp; <input type=\"radio\" name=\"FontSize\" value=12>Small");
//    out.println("          &nbsp;&nbsp; <input type=\"radio\" name=\"FontSize\" value=10>Tiny");
