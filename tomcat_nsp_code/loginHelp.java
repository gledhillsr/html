/* $Id: loginHelp.java,v 1.1.1.1 1999/10/09 00:19:59 duncan Exp $
 *
 */
...
//System.out.println("loginHelp id="+id+" pass="+pass+" parent="+szParent);
        response.setContentType("text/html");
        out = response.getWriter();

        if(isValidLogin(out, resort, id,pass,response)) {   //does password match?
//            CookieID cookie = new CookieID(request,response,null,"loginHelp");
        try {
//System.out.println("loginHelp id="+id+", szParent=("+szParent+"), resort=("+resort+")");
//            String newLoc = "http://" + PatrolData.IP_ADDRESS + "/"+resort+"/index.php?resort="+resort+"&NSPgoto="+szParent+"&ID="+id;
///            String newLoc = "http://127.0.0.1/"+resort+"/index.php?resort="+resort+"&NSPgoto="+szParent+"&ID="+id;
            String newLoc = "http://www.gledhills.com/"+resort+"/index.php?resort="+resort+"&NSPgoto="+szParent+"&ID="+id;
//System.out.println("..calling sendRedirect("+newLoc+")");
        ...
        if(emailme != null) {
            ...
            if(ID2 != null || email != null) {
                int ret = sendPassword(out,ID2, email, resort);
                if(ret == 0)
...
// sendPassword
//-------------------------
    public int sendPassword(PrintWriter out, String ID, String email, String resort){
        PatrolData patrol = new PatrolData(PatrolData.FETCH_ALL_DATA,resort);
        MemberData member = null;
        if(ID != null && !ID.equalsIgnoreCase(PatrolData.bdLogin) && ID.length() > 4) {
...
//out.println("FOUND ID=(" + ID + ")<BR>");
        ...
        if(member != null) {
            ...
            String from = email ;
            MailMan mail= new MailMan(smtp,from, fullName );
    ...
    public boolean isValidLogin(PrintWriter out, String resort,String ID,  String pass,HttpServletResponse response) {
    boolean validLogin = false;
    ResultSet rs;
//System.out.println("isValidLogin("+ID+", "+pass+")");
    if(ID == null || pass == null)
        return false;

    try  {
//System.out.println("loading driver");
//System.out.println("loginHelp: getting JData driver");
...
//System.out.println("driver loaded");
    ...
    if (ID.equalsIgnoreCase(PatrolData.bdLogin) &&  pass != null && pass.equalsIgnoreCase(PatrolData.bdPass)) {
...
//System.out.println("getting a DB connection for "+PatrolData.getJDBC_URL(resort));
...
//System.out.println("sRost="+sRost);
      if(sRost == null) {
        return false;
      }
...
//System.out.println("originalPassword=("+originalPassword+"), length="+originalPassword.length());
//System.out.println("  typed in password=("+pass+")");
//System.out.println("  lastName=("+lastName+")");
//System.out.println("  lastName.equalsIgnoreCase(pass)="+lastName.equalsIgnoreCase(pass));
        boolean hasPassword = (originalPassword != null && originalPassword.length() > 0);
//System.out.println("hasPassword="+hasPassword);
//System.out.println("lastName=("+lastName+"), pass=("+pass+")"); //name was passed on the command line as a quick password
...
//            response.addCookie(cookie);
      ...
      c.close();
        ...
        out.println("Error connecting or reading table:"+e.getMessage());
