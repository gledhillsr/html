...
//static data
    final static Hashtable lookupClassification=new Hashtable(10);
...
//instance data
    ...
    /***************/
    public MemberData() {
//System.out.println("Constructor-MemberData()");
        idNum = 0;
...
//System.out.println("Constructor-MemberData(request)");
        ...
        for(int i=0; i < DB_SIZE; ++i) {
            if(i == INSTRUCTOR) {
                int idx = 0;
                if(request.getParameter("OEC") != null)         idx += 1;
                if(request.getParameter("CPR") != null)         idx += 2;
                if(request.getParameter("SKI") != null)         idx += 4;
                if(request.getParameter("TOBOGGAN") != null)    idx += 8;
                memberData[i] = "" + idx;
...
//System.out.print(" ("+idx+")");
            ...
            String align = "";
            switch (idx) {
			    case BLANK:				str = "&nbsp";					break;
			    case BLANK_WIDE: 		str = "&nbsp";					break;
                case ID_NUM:            str = getID();                  break;
                case CLASSIFICATION:    str = getClassification();      break;
                case LAST:              str = getFullName2(); 			break;
                case FIRST:             str = getFullName();            break;
                case SPOUSE:            str = getSpouse();              break;
                case ADDRESS:           str = getAddress();             break;
                case CITY:              str = getCity();                break;
                case STATE:             str = getState();               break;
                case ZIPCODE:           str = getZipCode();             break;
                case HOMEPHONE:         str = getHomePhone();           break;
                case WORKPHONE:         str = getWorkPhone();           break;
                case CELLPHONE:         str = getCellPhone();           break;
                case PAGER:             str = getPager();               break;
                case EMAIL:
                    str = getEmail();
                    if(getEmail() != "&nbsp;")
                        str = "<a href=\"mailto:"+getEmail()+"\">"+getEmail()+"</a>";
                    break;
                case EMERGENCY:          str = getEmergency();          break;
                case PASSWORD:           str = getPassword();           break;
                case SUB:                str = getSub();                break;
                case COMMITMENT:         str = getCommitmentString(getCommitment()); break;
                case INSTRUCTOR:         str = getInstructorString(getInstructor()); break;
                case DIRECTOR:           str = getDirector();           break;
                case LAST_UPDATED:       str = getLastUpdated();        break;
                case COMMENTS:           str = getComments();           break;
//        CARRY_OVER_CREDITS,LAST_CREDIT_UPDATE,CAN_EARN_CREDITS, CREDITS_EARNED, CREDITS_USED, TEAM_LEAD, MENTORING }; //BRIGHTON ONLY
                case CARRY_OVER_CREDITS: str = getCarryOverCredits();   align = " align=center"; break;
                case LAST_CREDIT_UPDATE: str = getLastCreditDateStr();  break;
                case CAN_EARN_CREDITS:   str = getCanEarnCredits().equals("1") ? "Yes" : "No";  align = " align=center"; break;
                case CREDITS_EARNED:     str = getCreditsEarned();      align = " align=center"; break;
                case CREDITS_USED:       str = getCreditsUsed();        align = " align=center"; break;
                case TEAM_LEAD:          str = getTeamLead().equals("1") ? "Yes" : "No";   align = " align=center";  break;
                case MENTORING:          str = getMentoring().equals("1") ? "Yes" : "No";  align = " align=center";  break;

                case SHOW_NIGHT_CNT:     str = AssignmentCount[Assignments.NIGHT_TYPE] + "";  align = " align=center"; break;
                case SHOW_SWING_CNT:     str = AssignmentCount[Assignments.SWING_TYPE] + "";  align = " align=center"; break;
                case SHOW_DAY_CNT:       str = AssignmentCount[Assignments.DAY_TYPE] + "";    align = " align=center"; break;
                case SHOW_TRAINING_CNT:  str = AssignmentCount[Assignments.TRAINING_TYPE] + "";    align = " align=center"; break;
                case SHOW_NIGHT_LIST:	 str = szAssignments[Assignments.NIGHT_TYPE]; break;
                case SHOW_SWING_LIST:    str = szAssignments[Assignments.SWING_TYPE]; break;
                case SHOW_DAY_LIST:      str = szAssignments[Assignments.DAY_TYPE];   break;
                case SHOW_TRAINING_LIST:      str = szAssignments[Assignments.TRAINING_TYPE];   break;
            }
...
//System.out.println("=======");
    ...
    public static String getCommitmentString (String str) {
        if(str.equals("0"))      return "Inactive";  //no longer used
        else if(str.equals("1")) return "Part Time";
        return "Full Time";
    }
...
//System.out.println(i+") "+memberData[i]);
    ...
    private String readString(ResultSet rosterResults, String tag, String szDefault) {
        String str = null;
        try {
            str= rosterResults.getString(tag);
    ...
    public String getFullClassification() {
        ...
        try {
            classification = (String)lookupClassification.get(getClassification());
        } catch (Exception ex) {
...
//System.out.println("in getInsertSQLString, time=(string)"+str+" or (long)"+rightNow.getTimeInMillis());
        ...
        String qryString = "INSERT INTO Roster Values(";
        for(i=0; i < DB_SIZE; ++i) {
            if(i == DB_SIZE -1)
                qryString += "'"  + memberData[order[i]] + "')";
//              qryString += "\"" + dbData[i][DB_NAME]+"\" = '"  + memberData[i] + "')";
            else
                qryString += "'" + memberData[order[i]] + "', ";
//              qryString += "\"" + dbData[i][DB_NAME]+"\" = '" + memberData[i] + "', ";
        }
...
//System.out.println("in getUpdateSQLString, time=(string)"+str+" or (long)"+rightNow.getTimeInMillis());
        ...
        for(i=1; i < DB_SIZE; ++i) { //don't start with the key value (ID_NUM)
            if(order[i] == CARRY_OVER_CREDITS) {
                String ct = vouchersToCredits(memberData[order[i]]);
                qryString += dbData[order[i]][DB_NAME]+" = \""+ct+"\" ";
            } else
                qryString += dbData[order[i]][DB_NAME]+" = \""+memberData[order[i]]+"\" ";  
            if(i < DB_SIZE -1)
                qryString +=", ";
        }
        qryString += " WHERE "+dbData[ID_NUM][DB_NAME]+" ="+memberData[ID_NUM];
System.out.println("DB_SIZE="+DB_SIZE+" "+qryString);
        return qryString;
    }
...
//        ct = (int)(xx * 2.0);
          int xx = Integer.parseInt(vouchers);
          ct = (int)xx;
          credits = ct + "";
...
//System.out.println(qryString);
    ...
    private boolean testIfValid(String str) {
        ...
        if(str.equals("null"))
            return false;
        boolean valid = (!str.equals("") && !str.equals(" "));
        return valid;
    }
    ...
    /*******************************/
    public String getFullName()             { return memberData[FIRST] + " " + memberData[LAST];    }
    public String getFullName2()            { return memberData[LAST] + ", " + memberData[FIRST];    }
    public String getLast()                 { return memberData[LAST];  }
    public String getFirst()                { return memberData[FIRST];         }
    public String getSpouse()               { return memberData[SPOUSE];        }
    public String getAddress()              { return memberData[ADDRESS];       }
    public String getCity()                 { return memberData[CITY];          }
    public String getState()                { return memberData[STATE];         }
    public String getZipCode()              { return memberData[ZIPCODE];       }
    public String getHomePhone()            { return memberData[HOMEPHONE];     }
    public String getWorkPhone()            { return memberData[WORKPHONE];     }
    public String getCellPhone()            { return memberData[CELLPHONE];     }
    public String getPager()                { return memberData[PAGER];         }
    public String getEmail()                { return memberData[EMAIL];         }
    public String getClassification()       { return memberData[CLASSIFICATION];}
    public String getID()                   { return memberData[ID_NUM];        }
    public String getEmergency()            { return memberData[EMERGENCY];     }
    public String getPassword()             { return memberData[PASSWORD];      }
    public String getCommitment()           { return memberData[COMMITMENT];    }
    public String getInstructor()           { return memberData[INSTRUCTOR];    }
    public String getDirector()             { return memberData[DIRECTOR];      }
    public String getSub()                  { return memberData[SUB];           }
    public String getLastUpdated()          { return memberData[LAST_UPDATED];   }
    public String getComments()             { return memberData[COMMENTS];   }
    public String getCanEarnCredits()       { return memberData[CAN_EARN_CREDITS];  }
    public String getCreditsEarned()        { return memberData[CREDITS_EARNED];    }
    public String getCreditsUsed()          { return memberData[CREDITS_USED];  }
    public String getTeamLead()             { return memberData[TEAM_LEAD];     }
    public String getMentoring()            { return memberData[MENTORING];     }
    public String getCarryOverCredits()     { return memberData[CARRY_OVER_CREDITS]; }
    public String getLastCreditDate()       { return memberData[LAST_CREDIT_UPDATE]; }
    ...
    public void setLast(String str)             { memberData[LAST]         = str;  }
    public void setFirst(String str)            { memberData[FIRST]        = str;  }
    public void setSpouse(String str)           { memberData[SPOUSE]       = str;  }
    public void setAddress(String str)          { memberData[ADDRESS]      = str;  }
    public void setCity(String str)             { memberData[CITY]         = str;  }
    public void setState(String str)            { memberData[STATE]        = str;  }
    public void setZipCode(String str)          { memberData[ZIPCODE]      = str;  }
    public void setHomePhone(String str)        { memberData[HOMEPHONE]    = str;  }
    public void setWorkPhone(String str)        { memberData[WORKPHONE]    = str;  }
    public void setCellPhone(String str)        { memberData[CELLPHONE]    = str;  }
    public void setPager(String str)            { memberData[PAGER]        = str;  }
    public void setEmail(String str)            { memberData[EMAIL]        = str;  }
    public void setClassification(String str)   { memberData[CLASSIFICATION]= str; }
    public void setID(String str)               { memberData[ID_NUM]       = str;  }
    public void setEmergency(String str)        { memberData[EMERGENCY]    = str;  }
    public void setPassword(String str)         { memberData[PASSWORD]     = str;  }
    public void setCommitment(String str)       { memberData[COMMITMENT]   = str;  }
    public void setInstructor(String str)       { memberData[INSTRUCTOR]   = str;  }
    public void setDirector(String str)         { memberData[DIRECTOR]     = str;  }
    public void setSub(String str)              { memberData[SUB]          = str;  }
    public void setComments(String str)         { 
		str = str.replaceAll("\"","'");
		memberData[COMMENTS]     = str;  
	}
    public void setLastUpdated(String str)      { memberData[LAST_UPDATED]  = str;  }
    public void setCarryOverCredits(String str) { memberData[CARRY_OVER_CREDITS]  = str;     }
    public void setLastCreditDate(String str)   { memberData[LAST_CREDIT_UPDATE]  = str;  }
    public void setCanEarnCredits(String str)   { memberData[CAN_EARN_CREDITS]  = str;  }
...
//1st test classification
        if(vClassificationsToDisplay.indexOf(getClassification()) == -1) return false;
...
//System.out.println("commitment status = "+value);
        if(value == 3 || iCommitmentToDisplay == 0 || ((1<<value & iCommitmentToDisplay) == 0)) return false;
...
//System.out.println(member.getID()+ ") listDirector = "+listDirector+"  isDirector="+member.isDirector());
        ...
        if( listDirector && isDirector() )   ok = true;
        value = 0;
        try {
            str = getInstructor();
            value = Integer.parseInt(str);
        ...
        if( (instructorFlags & value) > 0 )     ok = true;
		if(!ok) return false;
// end of 3rd group ----
