...
echo "  <tr>\n";
echo "    <td width=\"20%\"><b>Top Shack(s)</B></td>\n";
echo "    <td width=\"20%\"><b>$top1Name</B></td>\n";
echo "    <td width=\"60%\"><b>$top1Time</B></td>\n";
echo "  </tr>\n";
if($top2Name != "None") {
    echo "  <tr>\n";
    echo "    <td width=\"20%\"><b>&nbsp;</B></td>\n";
    echo "    <td width=\"20%\"><b>$top2Name</B></td>\n";
    echo "    <td width=\"60%\"><b>$top2Time</B></td>\n";
    echo "  </tr>\n";
}
echo "  <tr>\n";
//..echo "    <td width=\"20%\"><br><b>Evening Sweep</B></td>\n";
echo "    <td width=\"20%\"><b>Evening Sweep</B></td>\n";
echo "    <td width=\"80%\" colspan=\"2\"><b>$eveningSweep</B></td>\n";
echo "  </tr>\n";
echo "</table>\n";
//
...
// Display new Page, and Title information
	...
	$pageNum = 1;
	if($j >= 0) echo "<h5>&nbsp;</h5>";	//start a new page

//	if($i == 0)
//		$topLoc = $getTopShack[$i];
//	else
//		$topLoc = $getTopShack[$i+1];  //skip Majestic top
	if($j <= 0)	//skip majestic
		$topLoc = $getTopShackDBName[$j+1];
	else
		$topLoc = $getTopShackDBName[$j+2];  //skip Majestic top
//    echo "$todayStr &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
//    echo "<font size=\"4\"><b>{$getArea[$i]}</b> Individual Assignments for $todayStr</font>";
...
// Build array of assigned sweeps, ordered by name
    ...
    $query_string = "SELECT sweep_ids, patroller_id FROM skihistory WHERE date=$today AND shift=0 AND areaID=$i ORDER BY name";
...
//
	$query_string = "SELECT * FROM sweepdefinitions WHERE areaID=$i ORDER BY description";
    $result = @mysql_db_query($mysql_db, $query_string) or die ("Invalid query 4");
    while ($row = @mysql_fetch_array($result)) {
        $id = $row[id];
		$location = $row[location];
		//if printing crest, and is snake, skip. 
//.		if($j == -1 && $location == $getTopShackDBName[1])	continue;
		//if printing snake, and is crest or aid room, skip
		if($j == 0 &&
			($location == $getTopShackDBName[5] ||
			 $location == $getTopShackDBName[6]) )	continue;

//        echo "area=$i, sweep id=" . $id . "--(" . $sweepsAssigned[$id] .")<br>";
//
        displaySweep($sweepsAssigned[$id], $id,$i,$topLoc);   //# 13 has a double top shack
    }
	if($i > 0) 
		echo "</p>";
}
