...
th.t1    {font-size:14; font-weight: bold; color:#000000; background-color:#E1E1E1; border-width:1px; border-color:#000000; border-style:solid; padding:2px}
td.t1    {font-size:12; color:#000000; background-color:#EBEBEB; border-width:1px; border-color:#000000; border-style:solid; padding:1px}

table.bas {border-width:0px border-style:none; border-collapse:collapse; border-spacing:0}
td.bas {border-width:0px border-style:none; border-collapse:collapse; border-spacing:0 font-size:12; color:#000000; background-color:none; padding:1px}
//-->
</style></head>
...
<?
  ...
  echo "</tr>\n";
    ...
    if($i == 1) //majestic
        $query_string = "SELECT * FROM sweepdefinitions WHERE location2=\"$loc\" ORDER BY start_time2";
    else
...
//  echo "Ordered by area and checkin time<br>";
  $patrollerCount = 0;
  $trainingCount = 0;
  $candidateCount = 0;
...
//echo "$query_string<br>";
                $result2 = @mysql_db_query($mysql_db, $query_string) or die ("Invalid query 22");
                if ($row2 = @mysql_fetch_array($result2)) {
