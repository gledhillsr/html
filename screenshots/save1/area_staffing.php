...
//========= show web column ========
	if($showWeb) {
     ...
     $result = @mysql_db_query($mysql_db, $query_string) or die ("Invalid query (result 10)");
     $lockerIDList=array();
     while ($row = @mysql_fetch_array($result)) 
     {
			$id = $row[ patroller_id];
      ...
      echo "  <td><select size=25 name=\"showWebLB\" style=\"font-size: 8pt; width: 133\">\n";
		...
		//open web connection
		$mysql_host = "gledhills.com"; //54.173.17.203";  //IP of nspOnline.org
		$connect_string = @mysqli_connect($mysql_host, $mysql_username, $mysql_password) or die ("Could not connect to web database.");
		//build date string
		  $tdate  = $arrDate[year] . "-";
        ...
        $result = @mysql_db_query($mysql_db, $query_string) or die ("Invalid query (result 11)");
	     $webIDList=array();
...
//zzz
	        			...
	        			$result2 = @mysql_db_query($mysql_db, $query_string) or die ("Invalid query (result 7)");
			        	if ($row2 = @mysql_fetch_array($result2)) {
