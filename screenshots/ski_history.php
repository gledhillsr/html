...
//echo "$query_string<br>";
    $result1 = @mysql_db_query($mysql_db, $query_string) or die ("Invalid query ($query_string)");
	$count = 0;
	$loginDetail = "";
...
//echo "query_string2=$query_string2<br>";
  	$result = @mysql_db_query($mysql_db, $query_string2) or die ("Invalid query2 ($query_string2)");
	$cnt = 0;
	$date1 = "";
...
//echo "done<br>";
	    ...
	    $result1 = @mysql_db_query($mysql_db, $query_string1) or die ("Invalid query4 ($query_string1)");
        $patrollers=array();
	    while ($row1 = @mysql_fetch_array($result1)) {
	        $name 	   = $row1[name];
			$pos = strpos($name," ");
...
//echo "$query_string<br>";
		    ...
		    $result = @mysql_db_query($mysql_db, $query_string) or die ("Invalid query5 ($query_string)");
...
//echo "showZero=$showZero<br>";
		if($showZero) {
		    ...
		    $result = @mysql_db_query($mysql_db, $query_string) or die ("Invalid query6 ($query_string)");
	    	while ($row = @mysql_fetch_array($result)) {
		        $ID = $row[IDNumber];
